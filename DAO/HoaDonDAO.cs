...
namespace QuanLyKhachHang.DAO
{
    class HoaDonDAO
    {
        ...
        {
            string query = string.Format("select h.MAHD, h.NGAYLAP, t.HOTEN,kh.TENKH, MAKH,dbo.PHANTRAMKM(MaKM) as PHANTRAM, h.TONGTIEN  from HOADON h, TAIKHOAN t, KHACHHANG kh, PHIEUYEUCAU p where (h.NGAYLAP between '{0}' and '{1}') and h.USERNAME = t.USERNAME and h.MAPYC = p.MAPYC and p.MAKH = kh.MAKH", ngaydau, ngaycuoi);
        ...
        {
            return DataProvider.Instance.executeQuery("select h.MAHD , h.NGAYLAP, t.HOTEN,kh.TENKH, MAKM, dbo.PHANTRAMKM(MaKM) as PHANTRAM, h.TONGTIEN from HOADON h, TAIKHOAN t, KHACHHANG kh, PHIEUYEUCAU p where h.USERNAME = t.USERNAME and h.MAPYC = p.MAPYC and p.MAKH = kh.MAKH");
        }
        ...
        {
            string query = string.Format("select h.MAHD, h.NGAYLAP, t.HOTEN, kh.TENKH, MAKM, dbo.PHANTRAMKM(MaKM) as PHANTRAM, h.TONGTIEN  from HOADON h, TAIKHOAN t, KHACHHANG kh, PHIEUYEUCAU p where (h.MAHD like '%'+'{0}'+'%' or '{0}'='') and (t.HOTEN like N'%'+N'{1}'+N'%' or N'{1}'='') and (h.NGAYLAP between '{2}' and '{3}') and h.USERNAME = t.USERNAME and h.MAPYC = p.MAPYC and p.MAKH = kh.MAKH", mahd, nguoilap, ngaydau, ngaycuoi);
            DataTable data = DataProvider.Instance.executeQuery(query);
            return data;
        }
        ...
        {
            string query = string.Format("select h.MAHD, h.NGAYLAP, t.HOTEN,kh.TENKH, MAKM, dbo.PHANTRAMKM(MaKM) as PHANTRAM, h.TONGTIEN from HOADON h, TAIKHOAN t, KHACHHANG kh, PHIEUYEUCAU p where (h.MAHD like '%' + '{0}' +'%' or '{0}'='' ) and (t.HOTEN like N'%' + N'{1}' +N'%' or N'{1}'='' ) and h.USERNAME = t.USERNAME and h.MAPYC = p.MAPYC and p.MAKH = kh.MAKH", mahd, nguoilap);
            DataTable data = DataProvider.Instance.executeQuery(query);
            return data;
        }
        ...
        {
            string query = string.Format("select h.MAHD, h.NGAYLAP, t.HOTEN,kh.TENKH, MAKM, dbo.PHANTRAMKM(MaKM) as PHANTRAM, h.TONGTIEN from HOADON h, TAIKHOAN t, KHACHHANG kh, PHIEUYEUCAU p where (h.MAHD like '%' + '{0}' +'%' or '{0}'='' ) and (t.HOTEN like N'%' + N'{1}' +N'%' or N'{1}'='' ) and (h.NGAYLAP >= '{2}' ) and h.USERNAME = t.USERNAME and h.MAPYC = p.MAPYC and p.MAKH = kh.MAKH", mahd, nguoilap, ngaydau);
            DataTable data = DataProvider.Instance.executeQuery(query);
            return data;
        }
